 ...
 *
 * One canonical event shape is produced for every provider transport:
 *
 *   { type, requestId, runId, data }
 *
 * type: "text_delta" | "reasoning_delta" | "tool_call_start" |
 *       "tool_call_delta" | "tool_call_end" | "usage" | "finish" | "error"
 *
 * The normalizer assembles fragmented tool-call JSON, deduplicates finish
 * and usage events, and rejects events that arrive after the stream has
 * already completed or failed. Partial content is never committed here:
 * callers only receive a finish event after a validated completion.
 */
