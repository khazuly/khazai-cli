...
export function getOpenCodeCredential(
  ...
  const opencodeAuth = readAuth(path).opencode;
  if (opencodeAuth && typeof opencodeAuth === "object") {
    const value = String(opencodeAuth.key || opencodeAuth.apiKey || opencodeAuth.token || "").trim();
    if (value) return value;
  }
  const khazaiAuthPath = join(homedir(), ".local", "share", "khazai-ai", "auth.json");
  const khazaiAuth = readAuth(khazaiAuthPath).opencode;
  if (khazaiAuth && typeof khazaiAuth === "object") {
    return String(khazaiAuth.value || khazaiAuth.key || khazaiAuth.apiKey || khazaiAuth.token || "").trim();
  }
  return "";
}
