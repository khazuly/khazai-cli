...
export function getOpenCodeCredential(path = join(homedir(), ".local", "share", "opencode", "auth.json")) {
  const entry = readAuth(path).opencode;
  if (!entry || typeof entry !== "object") return "";
  return String(entry.key || entry.apiKey || entry.token || "");
}
