...
export function resolveProvider(model, config = loadConfig()) {
  ...
  return {
    provider: new OpenAICompatibleProvider({
      ...
      rotateClientIp: descriptor.definition.rotatingTransport === true,
      transportKey: `${descriptor.providerID}:${descriptor.modelID}`,
      publicName: descriptor.definition.rotatingTransport ? "KhazAI" : descriptor.providerID,
      hideUpstreamDiagnostics: descriptor.definition.rotatingTransport === true,
...
function activeTransport(provider, options) {
  return !provider.transportIsCurrent
    || provider.transportIsCurrent(options.transportRevision, options.rotationId);
}
...
export async function chatWithRetry(provider, messages, options, attempts = 3) {
  ...
  for (let attempt = 0; attempt < maximumAttempts; attempt++) {
    ...
    try {
      ...
      if (!activeTransport(provider, activeAttemptOptions)) {
        const stale = new Error("Provider transport changed before the request completed.");
        stale.code = "STALE_TRANSPORT";
        stale.failureClass = "premature_stream";
        stale.retryAfterMs = 0;
        throw stale;
      }
      ...
      if (!activeTransport(provider, activeAttemptOptions)) {
        error.code ||= "STALE_TRANSPORT";
        error.failureClass ||= "premature_stream";
      }
