...
export async function chat(messages, opts = {}) {
  ...
  if (contextLimit) {
    // The agent may already have projected the normalized payload for this
    // exact revision (and compacted at the safe boundary). In that case the
    // projection is reused and the unchanged history is not normalized,
    // validated, and re-serialized a second time inside the request path.
    ...
    if (
      ...
      if (fallbackRoute) {
        ...
        debugProviderDiagnostic({
          ...
          alias: fallbackRoute.alias,
          routeId: fallbackRoute.routeId,
          upstreamModel: fallbackRoute.upstreamModel,
          adapter: fallbackRoute.adapterId,
          routeFingerprint: fallbackRoute.fingerprintKey,
          failureClass: "fallback_selected",
          fallbackSelected: fallbackRoute.alias,
          attempt: 0,
        });
