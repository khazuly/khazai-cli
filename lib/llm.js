...
import { MistralLeChatProvider } from "./mistral-provider.js";
import { AIChatProvider } from "./aichat-provider.js";
import { GeminiGuestProvider } from "./gemini-guest-provider.js";
import { mergeToolCallDelta, OpenAICompatibleProvider } from "./providers.js";
import { qwenCloudClient, QWEN_CLOUD_ID } from "./qwen-cloud.js";
...
function routeOptions(options, route) {
  return route ? { ...options, route, alias: route.alias, routeId: route.routeId, upstreamModel: route.upstreamModel } : options;
}
...
export function resolveProvider(model, config = loadConfig()) {
  ...
  if (descriptor.providerID === "mistral") {
    return {
      provider: new MistralLeChatProvider(),
      model: descriptor.modelID,
      id: "mistral",
      requested: descriptor.requested,
      exactID: descriptor.exactID,
  ...
  if (descriptor.providerID === "aichat") {
    return {
      provider: new AIChatProvider({ baseURL: descriptor.definition.baseURL }),
      model: descriptor.modelID,
      id: "aichat",
      requested: descriptor.requested,
      exactID: descriptor.exactID,
  ...
  if (descriptor.providerID === "gemini-guest") {
    return {
      provider: new GeminiGuestProvider(),
      model: descriptor.modelID,
      id: "gemini-guest",
      requested: descriptor.requested,
      exactID: descriptor.exactID,
  ...
  return {
    provider: descriptor.providerID === QWEN_CLOUD_ID
      ? qwenCloudClient({ apiKey, baseURL: descriptor.definition.baseURL })
...
function canRetryFreeModelAnonymously(resolved, error) {
  return ["opencode", "khazai-free"].includes(resolved.id)
    && Boolean(resolved.apiKey)
...
function anonymousProvider(resolved) {
  return new OpenAICompatibleProvider({
    ...
    compatibility: resolved.definition.compatibility || {},
  });
}
...
async function chatWithFreeRoutes(messages, options, attempts, config, legacy) {
  let fallbackError;
  const gatewayRoute = resolveRoute("auto-free", config);
  try {
    return await chatWithRetry(legacy.provider, messages, routeOptions(options, gatewayRoute), attempts);
  } catch (error) {
    fallbackError = error;
    if (canRetryAutoFreeAnonymously(legacy, error)) {
      try {
        return await chatWithRetry(anonymousProvider(legacy), messages, options, 1);
      } catch (anonymousError) {
        fallbackError = anonymousError;
      }
    }
  }
  const settings = resolveEffectiveSettings("auto-free", { config });
  const requirements = { tools: (options.tools?.length || 0) > 0, preferReasoning: true,
  ...
  for (const candidate of candidates) {
    ...
    try {
      const result = await chatWithRetry(resolved.provider, messages,
        routeOptions(freeRouteOptions(resolved, options, candidate, config), candidateRoute), attempts);
      if (failedRoutes > 0) {
        options.onEvent?.({ type: "provider-fallback", model: candidate.displayName });
      }
      return result;
    } catch (error) {
      failedRoutes++;
      fallbackError = error;
      error.resolvedModel = candidate.displayName;
...
export async function chat(messages, opts = {}) {
  ...
  const alias = String(requestOptions.model || config.model || "auto-free");
  ...
  try {
    ...
    error.resolvedModel = canonicalModelKey(alias, config);
    error.totalAttempts = Number(error.attempts || 1);
    if (canRetryFreeModelAnonymously(resolved, error)) {
    ...
    if (
      ...
      if (fallbackRoute) {
        ...
        requestOptions.onEvent?.({ type: "provider-fallback", model: canonicalModelKey(fallbackRoute.alias, config) });
        ...
        const fallbackAttempts = Math.max(1, Number(maxProviderAttempts ?? fallbackSettings.maxProviderRetries) || 1);
        try {
          return await chatWithRetry(fallback.provider, messages, fallbackOptions, fallbackAttempts);
        } catch (fallbackError) {
          fallbackError.resolvedModel = canonicalModelKey(fallbackRoute.alias, config);
...
export function estimateProviderPayload(messages, tools = [], model = "auto-free", config = null) {
