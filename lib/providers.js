  ...
  async chat(messages, {
    ...
    try {
      ...
      const body = {
        ...
        ...(includeUsage && capabilities.supportsStreamOptions !== false
          ? { stream_options: { include_usage: true } }
          : {}),
        ...(Number.isFinite(maxTokens) && capabilities.supportsMaxTokens !== false
          ? { max_tokens: maxTokens }
          : {}),
        ...(temperature !== undefined && capabilities.supportsTemperature !== false
          ? { temperature }
          : {}),
        ...(topP !== undefined && capabilities.supportsTopP !== false ? { top_p: topP } : {}),
        ...(requestTools.length ? {
          tools: requestTools,
          ...(toolChoice && capabilities.supportsToolChoice !== false ? { tool_choice: toolChoice } : {}),
          ...(parallelToolCalls !== undefined && capabilities.supportsParallelTools
        ...
        await readSse(response.body, data => {
          if (data.usage) {
            onEvent?.({
              ...
              contextLimit: data.context_limit ?? data.context_window,
            });
          }
