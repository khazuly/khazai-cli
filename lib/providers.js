...
import { sanitizeAssistantIdentity } from "./assistant-text.js";export function mergeToolCallDelta(calls, delta = {}) {
...
/**
 * OpenAI-compatible provider built on the shared provider adapter.
 * Request building, message normalization, stream parsing, usage
 * normalization, and error classification all live in the adapter so the
 * canonical pipeline never duplicates provider-specific logic.
 */
...
export class OpenAICompatibleProvider {
  constructor({ id, baseURL, apiKey = "", headers = {}, rotateSessionAffinity = false }) {
    ...
    this.adapter = createOpenAICompatibleAdapter({ id, baseURL: this.baseURL });
  }
    ...
    try {
      ...
      if (body.stream && response.body && /text\/event-stream/i.test(contentType)) {
        const result = await adapter.parseStream(response, {
          ...
          onToken,
          model,
          requestId: optionsRequestId(headers),
          includeUsage,
        });
        return result.text;
      }
...
function optionsRequestId(headers) {
  return String(headers?.["x-opencode-request"] || "");
}
