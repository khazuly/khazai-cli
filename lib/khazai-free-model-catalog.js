...
/**
 * Health state is read from both the legacy route keys (provider + model +
 * endpoint) and the stable route fingerprint. Both are written by the
 * request pipeline so the model selector reflects the real health of the
 * resolved upstream route, and discovery never masks chat failures.
 */
function modelFingerprint(model) {
  const isBigCock = model.upstreamModel === "big-pickle";
  return {
...
export async function refreshZenAvailability({
  ...
  try {
    ...
    // Discovery marks a model as discoverable but never resets chat-failure
    // health. A route only returns to "healthy" after a successful request.
  } catch (error) {
    cache = {
      checkedAt: now,
