const SECRET_PATTERNS = [
  ...
  /\b(?:api[_ -]?key|token|password|credential|secret|cookie|authorization)\s*(?:is\s*)?(?:[:=]|\s+)\s*[^\s"'&,;]+/gi,
];

export function redactSecrets(value) {
  let text = String(value ?? "");
  for (const pattern of SECRET_PATTERNS) text = text.replace(pattern, match => {
    const prefix = /^(?:Bearer\s+|(?:api[_ -]?key|token|password|credential|secret|cookie|authorization)\s*(?:is\s*)?(?:[:=]|\s+)\s*)/i.exec(match)?.[0] || "";
    return `${prefix}[REDACTED]`;
  });
  return text;
}

// Redact JSON-compatible data without editing its serialized representation.
// Applying redactSecrets() to JSON.stringify(value) can replace an escaped
// quote in a string value and leave invalid JSON behind.
