...
import { syntheticPartialToolCall } from "./tool-call-buffer.js";
...
function responseError(status, statusText, detail = "", headers = null) {
  const error = new Error(`HTTP ${status}: ${statusText || "AIChat request failed"}`);
  error.status = status;
  error.providerDetail = String(detail).slice(0, 1_000);
  if (status === 429) error.failureClass = "rate_limited";
  else if ([408, 425].includes(status) || status >= 500) error.failureClass = "provider_infrastructure";
...
function normalizeToolReply(output) {
  const candidates = [
    output.trim(),
    ...[...output.matchAll(/```(?:json)?\s*([\s\S]*?)```/gi)].map(match => match[1]?.trim()),
    ...embeddedJson(output),
  ];
...
async function readSSE(response, { onToken, onEvent, deferTokens = false, ...scope }) {
  ...
  const partial = deferTokens && result === output ? syntheticPartialToolCall(output, {
  ...
  if (partial) {
    onEvent?.({ type: "partial-tool-call", partial });
    return "";
  }
...
export class AIChatSessionPool {
  ...
  async establish() {
    ...
    if (!response.ok) throw responseError(response.status, response.statusText, await responseDetail(response), response.headers);
    ...
    this.trim();
    return { csrf: match[1], cookies, version: 1, lastUsedAt: Date.now(), usage: null, usageCheckedAt: 0 };
  }
...
export class AIChatProvider {
  ...
  async usage(session) {
    if (Date.now() - Number(session.usageCheckedAt || 0) < 30_000) return session.usage || null;
    try {
      const response = await this.fetch(`${this.baseURL}/api/chat/usage`, {
        headers: this.headers(session, { Accept: "application/json" }),
      });
      session.usageCheckedAt = Date.now();
      if (!response.ok) return session.usage || null;
      session.usage = await response.json();
    ...
    return this.sessionPool.withSession(sessionId, async session => {
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        try {
          ...
          session = await this.sessionPool.rotate(sessionId);
        }
      }
    });
  }
