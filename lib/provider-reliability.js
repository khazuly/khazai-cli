...
export function providerDiagnostic(provider, options, error, attempt, failureClass, retryDelayMs = null, extra = {}) {
  ...
  const hideUpstream = provider.hideUpstreamDiagnostics === true;
  return {
    requestId: options.requestId || error?.requestId || "",
    provider: provider.publicName || provider.id || "unknown",
    model: hideUpstream ? extra.alias || options.alias || "" : options.model || "",
    alias: extra.alias || options.alias || "",
    routeId: extra.routeId || options.routeId || "",
    upstreamModel: hideUpstream ? "" : options.upstreamModel || options.model || "",
    adapter: extra.adapter || provider.adapterId || "unknown",
    routeFingerprint: hideUpstream ? "" : fingerprint?.fingerprintKey
      || (fingerprint ? `${fingerprint.providerId}|${fingerprint.baseURL}|${fingerprint.upstreamModel}|${fingerprint.credentialPoolId}` : ""),
    endpoint: hideUpstream ? "" : error?.endpoint || provider.chatURL?.() || "",
    attempt,
    ...
    transportRevision: Number(options.transportRevision) || null,
