...
export function classifyProviderFailure(error, { toolCount = 0, plainRouteHealthy = false } = {}) {
  ...
  if (/api[ _-]?key|credential|authentication|unauthorized|forbidden|access denied/i.test(detail)) {
  ...
  if ([408, 425].includes(status)) return "provider_infrastructure";
  if (
  ...
  if (status >= 500) return "provider_infrastructure";
  if (["ECONNRESET", "EPIPE", "ETIMEDOUT", "ECONNREFUSED", "ENETUNREACH", "UND_ERR_SOCKET", "UND_ERR_CONNECT_TIMEOUT"].includes(code)) {
    return "provider_infrastructure";
  }
  if (/connection reset|socket hang up|fetch failed|network error|timed out|timeout/i.test(detail)) {
    return "provider_infrastructure";
  }
...
export function isRetryableFailure(failureClass) {
  return ["provider_infrastructure", "premature_stream", "rate_limited"].includes(failureClass);
}
