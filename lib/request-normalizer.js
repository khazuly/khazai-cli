...
/**
 * Canonical request pipeline helpers.
 *
 * The pipeline is: canonical messages -> validate sequence -> resolve alias
 * -> resolve route -> resolve capabilities -> normalize -> transform tools
 * and options -> context preflight -> build request -> stream -> normalize
 * events -> commit canonical history.
 *
 * These helpers are provider-neutral. Provider-specific conversion happens
 * only inside the resolved provider adapter.
 */
