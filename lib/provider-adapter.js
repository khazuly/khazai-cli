...
import { proseLooksIncomplete } from "../app/agent/helpers/parser.js";

/**
 * Provider adapter interface:
 *
 *   normalizeMessages(canonicalMessages, capabilities)
 *   normalizeTools(tools, capabilities)
 *   buildRequest({ model, messages, tools, options, capabilities })
 *   parseStream(response, callbacks, options)
 *   normalizeUsage(payload, model)
 *   classifyError(error, context)
 *
 * Shared OpenAI-compatible behavior lives here once. Narrow per-route
 * compatibility transforms are supplied through the `compatibility`
 * option instead of living in UI components or aliases.
 */
...
export async function parseOpenAIStream(response, {
  ...
  try {
    completed = await readSse(
      response.body,
      chunk => {
        for (const event of normalizer.handleChunk(chunk)) emitLegacy(event);
      },
