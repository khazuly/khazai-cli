...
import {
  canonicalLlmProxyToolText,
  normalizeLlmProxyMessage,
  parseLlmProxyStream,
  promptToolMessages,
} from "./prompt-tool-protocol.js";
...
export function buildChatRequestBody({
  ...
  if (compatibility.toolProtocol === "prompt-json" && tools.length) {
    messages = promptToolMessages(messages, tools);
    tools = [];
  }
  const body = {
    model,
...
export function createOpenAICompatibleAdapter({
  ...
  return {
    ...
    normalizeResponseMessage: (message, options) => compatibility.toolProtocol === "prompt-json"
      ? normalizeLlmProxyMessage(message, options?.requestId)
      : message,
    parseStream: (response, callbacks, options) => compatibility.toolProtocol === "prompt-json"
      ? parseLlmProxyStream(response, callbacks, options, parseOpenAIStream)
      : parseOpenAIStream(response, { ...callbacks, ...options }),
    normalizeUsage,
    classifyError,
    canonicalToolText: compatibility.toolProtocol === "prompt-json"
      ? canonicalLlmProxyToolText
      : canonicalToolText,
