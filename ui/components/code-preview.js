...
function HighlightedLine({ row, language, lineNumber, width }) {
  ...
  const parts = tokenizeLine(text, language);
...
function MetaLine({ text, color, width }) {
  const content = fitLine(` ${text}`, Math.max(1, width));
  return h(Text, { color, backgroundColor: COLORS.background }, content.padEnd(width));
}
...
function SyntaxPanel({ title, language, rows }) {
  ...
  return h(Box, {
    ...
    ...limitRows(rows).map((row, index) => {
      if (row.type === "meta-delete") return h(MetaLine, { key: index, text: row.text, color: COLORS.deleted, width });
      if (row.type === "meta-add") return h(MetaLine, { key: index, text: row.text, color: COLORS.added, width });
      if (row.type === "hunk") return h(MetaLine, { key: index, text: row.text, color: COLORS.hunk, width });
      if (row.type === "omitted") return h(MetaLine, { key: index, text: row.text, color: COLORS.muted, width });
      return h(HighlightedLine, { key: index, row, language, lineNumber: index, width });
    }))
  );
}
