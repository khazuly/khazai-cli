...
export { handlePermissionCommand };
...
export async function handleSessionCommand(cmd, arg, context) {
  ...
  if (cmd === "/allow-all" || cmd === "/auto") {
    const requested = cmd === "/auto" && !String(arg || "").trim()
      ? (context.autoApproveRef?.current ? "off" : "on")
      : arg;
    return handlePermissionCommand(cmd === "/auto" ? "/allow-all" : cmd, requested, {
      ...context,
      permissionService: context.permissionService
        || (agentRef.current?._permissionService
          ? () => agentRef.current._permissionService
          : () => new PermissionService(workspacePath)),
    });
  }
  if (cmd === "/permissions") {
    return handlePermissionCommand(cmd, arg, {
      ...context,
      permissionService: context.permissionService
        || (agentRef.current?._permissionService
          ? () => agentRef.current._permissionService
