...
let exaClient = null;
let exaTransport = null;

function normalizeApiResults(values = []) {
  const seen = new Set();
  return values.flatMap(value => {
    const url = normalizeResultUrl(value.url || value.link || value.href);
    const title = decodeHtml(value.title || value.name || "");
    if (!url || !title || seen.has(url)) return [];
    seen.add(url);
    return [{
      title,
      url,
      snippet: decodeHtml(value.snippet || value.description || value.text || ""),
      domain: new URL(url).hostname.replace(/^www\./, ""),
    }];
  });
}

function parseExaContent(result) {
  const text = (result?.content || []).filter(item => item.type === "text").map(item => item.text).join("\n");
  for (const candidate of [text, /\{[\s\S]*\}/.exec(text)?.[0]]) {
    if (!candidate) continue;
    try {
      const parsed = JSON.parse(candidate);
      const normalized = normalizeApiResults(parsed.results || parsed.data || (Array.isArray(parsed) ? parsed : []));
      if (normalized.length) return normalized;
    } catch {}
  }
  return normalizeApiResults([...text.matchAll(/\[([^\]]+)\]\((https?:\/\/[^)\s]+)\)/g)]
    .map(match => ({ title: match[1], url: match[2] })));
}

async function searchExa(query, count) {
  if (!exaClient) {
    exaTransport = new StreamableHTTPClientTransport(new URL("https://mcp.exa.ai/mcp"), {
      reconnectionOptions: {
    ...
    exaClient = new Client({ name: "khazai-ai-websearch", version: "0.3.0" }, { capabilities: {} });
    try { await exaClient.connect(exaTransport, { timeout: 5000 }); }
    catch (error) {
      exaClient = null;
      try { await exaTransport.close(); } catch {}
      exaTransport = null;
      throw error;
    }
  }
  ...
  const normalized = parseExaContent(result);
  if (!normalized.length) throw new Error("Exa returned no parseable results");
  return normalized;
}

async function searchApi(provider, query, count) {
  if (provider === "brave" && process.env.BRAVE_API_KEY) {
    ...
    return normalizeApiResults((await response.json()).web?.results);
  }
  if (provider === "tavily" && process.env.TAVILY_API_KEY) {
    ...
    return normalizeApiResults((await response.json()).results);
  }
  if (provider === "serper" && process.env.SERPER_API_KEY) {
    ...
    return normalizeApiResults((await response.json()).organic);
  }
  throw new Error(`${provider} is not configured`);
}

async function searchProviders(query, count) {
  const configured = loadConfig().web?.search?.providers;
  ...
  for (const provider of providers) {
    try {
      const results = provider === "exa"
        ...
        : provider === "local" || provider === "duckduckgo" || provider === "bing"
          ? await searchLocal(query)
          : await searchApi(provider, query, count);
...
export async function shutdownWebSearch() {
  try { await exaClient?.close(); } catch {}
  try { await exaTransport?.close(); } catch {}
  exaClient = null;
  exaTransport = null;
}
...
export const webSearchTool = {
  ...
  async execute({ query, count = 5 }) {
    ...
    try {
      ...
      if (!results) {
        ({ results } = await searchProviders(normalizedQuery, limit));
      ...
      if (domain && relevant.length === 0 && normalizedQuery.toLowerCase() !== domain) {
        ...
        if (!fallbackResults) {
          ({ results: fallbackResults } = await searchProviders(domain, limit));
