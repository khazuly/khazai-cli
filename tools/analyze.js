...
function workspacePath(path = ".", workspace = process.cwd(), allowExternal = false) {
  const absolute = resolve(workspace, String(path));
  if (!allowExternal && !isInsideWorkspace(absolute, workspace)) {
...
export const analyzeTool = {
  ...
  async execute({ path = ".", error = "", query = "", _agentWorkspace, _allowExternal } = {}) {
    const workspace = resolve(_agentWorkspace || process.cwd());
    let target;
    try {
      target = workspacePath(path, workspace, _allowExternal);
    } catch (err) {
