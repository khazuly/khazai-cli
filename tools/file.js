...
function isInstalled(mod, ext) {
  const cacheKey = `${process.cwd()}\0${ext}\0${mod}`;
  if ([".js", ".mjs", ".cjs", ".jsx", ".ts", ".tsx"].includes(ext)) {
    const base = mod.startsWith("@") ? mod.split("/").slice(0, 2).join("/") : mod.split("/")[0];
    const jsCacheKey = `${process.cwd()}\0${ext}\0${base}`;
    if (MODULE_CACHE.has(jsCacheKey)) return MODULE_CACHE.get(jsCacheKey);
    try {
      require.resolve(base, { paths: [process.cwd()] });
      MODULE_CACHE.set(jsCacheKey, true);
      return true;
    } catch {
      MODULE_CACHE.set(jsCacheKey, false);
      return false;
    }
  }
  if ([".py", ".pyw"].includes(ext)) {
    ...
    for (const [prog, args] of candidates) {
      ...
      if (r.status === 0) {
        MODULE_CACHE.set(cacheKey, true);
        return true;
      }
    }
    MODULE_CACHE.set(cacheKey, false);
    return false;
  }
  return true;
}
...
export function requireWorkspace(path, agentWorkspace, allowExternal = false) {
...
export function validateSource(path, content) {
...
export const readTool = {
  ...
  async execute({ path, offset = 1, limit = 2000, _agentWorkspace, _allowExternal }) {
    requireWorkspace(path, _agentWorkspace, _allowExternal);
    const abs = resolve(process.cwd(), String(path));
    if (!existsSync(abs)) return `Error: not found: ${path}`;
    if (statSync(abs).isDirectory()) return `Error: ${path} is a directory`;
...
export const writeTool = {
  ...
  async execute({ path, content, _agentWorkspace, _allowExternal }) {
    requireWorkspace(path, _agentWorkspace, _allowExternal);
    const abs = resolve(process.cwd(), String(path));
    ...
    return `Written ${Buffer.byteLength(next)} bytes to ${path}`;
  },
};
...
export const editTool = {
  ...
  async execute({ path, oldString, newString, _agentWorkspace, _allowExternal }) {
    requireWorkspace(path, _agentWorkspace, _allowExternal);
    const abs = resolve(process.cwd(), String(path));
    if (!existsSync(abs)) return `Error: not found: ${path}`;
    if (!String(oldString)) return `Error: oldString must not be empty`;
    if (String(oldString) === String(newString)) return `No changes to ${path}`;
    const original = readFileSync(abs, "utf-8");
    const matches = original.split(String(oldString)).length - 1;
    if (matches === 0) return `Error: oldString not found in ${path}`;
    if (matches > 1) return `Error: oldString is not unique in ${path}`;
    const updated = original.replace(String(oldString), String(newString));
