...
export const readTool = {
  ...
  async execute({ path, offset = 1, limit = 2000, _agentWorkspace }) {
    ...
    if (statSync(abs).isDirectory()) return `Error: ${path} is a directory. Use glob to list files inside it.`;
...
export const writeTool = {
  ...
  async execute({ path, content, _agentWorkspace }) {
    ...
    // A failed validation must never leave a partially-invalid file on disk.
