...
import {
  ...
  setMistralSessionFactory,
} from "../lib/mistral-provider.js";

const CHAT_ID = "260d38f4-e258-4a49-ac8d-6aedb27585b5";
const MSG_ID = "c5716078-5a8d-4769-b2eb-e224744a3194";
...
function newChatResponse(chatId = CHAT_ID, msgId = MSG_ID) {
...
function withFactory(routes, fn) {
  const tracker = { created: 0, landings: 0, closes: 0 };
  const original = setMistralSessionFactory;
  setMistralSessionFactory(async () => fakeSession(routes, tracker));
  return Promise.resolve(fn(tracker)).finally(() => setMistralSessionFactory(original));
}

function liveRoutes(requests, options = {}) {
  return [
